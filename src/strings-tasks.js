...
function countVowels(str) {
  ...
  return lowerCase.filter((item) => vowels.includes(item)).length;
}
...
function isPalindrome(str) {
  const strToLow = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  return strToLow === strToLow.split('').reverse().join('');
}
...
function reverseWords(str) {
  ...
  const reverseArray = array.map((elem) => elem.split('').reverse().join(''));
...
function encodeToRot13(str) {
  const normalAl = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const strangeAl = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[A-Z]/gi, (elem) => strangeAl[normalAl.indexOf(elem)]);
}
/**
